...
const DivisionSaleChartReport = ({ customers }: Props) => {
  ...
  const customersWithoutOthers = customers.filter(
    (c) => c.customer_name !== "Others"
  );
  ...
  const dynamicOthersCount = remainingCustomers.reduce(
  ...
  const combinedOthers = {
    ...
    total_order_value:
      (Number(othersFromBackend?.total_order_value) ?? 0) +
      dynamicOthersOrderTotal,
    color: "#F56630",
  };
            ...
            <div className="flex gap-3">
              <Text size="md" fw={400}>
                {data.name === "Others"
                  ...
                  : topCustomers.find((c) => c.customer_name === data.name)
                      ?.total_orders ?? 0}
              </Text>
