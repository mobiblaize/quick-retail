...
const SalesCustomerAnalysis = ({ reportInfo }: SalesCustomerAnalysisProps) => {
  ...
  return (
    ...
    <section className="w-full lg:flex-1 min-w-[320px] h-auto px-4 sm:px-6 py-6 sm:py-8 rounded-lg bg-white">
      ...
      {/* Scrollable product list for smaller screens */}
      <div className="mt-6 flex flex-col gap-4 overflow-x-auto">
        {topProducts.map((product, index) => (
          <div
            key={index}
            className="flex justify-between items-center px-2 py-2 hover:bg-gray-100 rounded gap-2 sm:gap-4 min-w-[280px]"
          >
            ...
            {/* Right: Price & Sold */}
            <div className="flex gap-3 sm:gap-6 items-center justify-end text-right flex-shrink-0">
              <Text
                fw={400}
                size="sm"
                c="black"
                className="min-w-[70px] sm:min-w-[80px] text-right"
              >
        ...
        {otherSummary.quantity_sold > 0 && (
          <div className="flex justify-between items-center px-2 py-2 bg-gray-50 rounded min-w-[280px]">
            <div className="flex gap-2 items-center flex-1 min-w-0">
              <img
                src={others}
                alt="Other Products"
