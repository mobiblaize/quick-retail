...
const ReportDateSelect = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { reportType, reportLabel } = (location.state as any) || {};
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");
  const generateReport = useGenerateReport();
  const [loading, setLoading] = useState(false);
    ...
    <>
      ...
      <div className="flex items-center justify-center bg-[#F2F4F7] px-4 py-6">
        <div className="w-full max-w-md bg-white p-4 sm:p-8 rounded-xl shadow-md">
          ...
          <div className="mb-4">
            <Stack gap="sm" w="100%">
              <Text fw={500} size="sm" c="gray.7">Stores Filter</Text>
              <Dropdown2
                options={storeOptions}
          ...
          <div className="mb-4">
            <Text size="sm" c="gray.6" mb={4}>Start Date</Text>
            <DateInput
              value={startDate ? new Date(startDate) : null}
              onChange={(date) => setStartDate(date ? date.toISOString().split("T")[0] : "")}
              placeholder="Select date"
          ...
          <div className="mb-6">
            <Text size="sm" c="gray.6" mb={4}>End Date</Text>
            <DateInput
              value={endDate ? new Date(endDate) : null}
              onChange={(date) => setEndDate(date ? date.toISOString().split("T")[0] : "")}
              placeholder="Select date"
          ...
          >
            <Text fw={500} size="sm" className="text-sm" c="#fff">
