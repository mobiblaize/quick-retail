...
import { financialMetricsOverviewData, financialMetricsOverviewData2 } from "../../../../utils/mockData";
import { useEffect, useState } from "react";
import DateFilterMenu from "../../../General/filterMenu";
import AnalyticsCard from "../../../General/card";

const FinancialMetricsBox = () => {
  ...
  return (
    <main className="w-full h-auto overflow-auto px-3 sm:px-6 py-4 sm:py-8 rounded-lg bg-white">
      ...
      <section className="flex flex-col sm:flex-row overflow-auto gap-6 md:gap-2 mt-5">
        {financialMetricsOverviewData.map((card, index) => (
          <AnalyticsCard
            key={index}
            title={card.title}
            value={card.value}
      ...
      <section className="flex flex-col sm:flex-row overflow-auto gap-6 md:gap-2 mt-5">
        {financialMetricsOverviewData2.map((card, index) => (
          <AnalyticsCard
            key={index}
            title={card.title}
            value={card.value}
...
export default FinancialMetricsBox;
